...
class TrajectoryService:
    ...
    async def create_and_process_async(self, trajectory_create: TrajectoryCreate, auto_process: bool = True) -> Trajectory:
        """
        ...
        if auto_process:
            task = asyncio.create_task(self._process_trajectory_background(trajectory.id))
            self._processing_tasks[trajectory.id] = task
            logger.info(f"Spawned background processing task for trajectory {trajectory.id}")

        return trajectory

    async def _process_trajectory_background(self, trajectory_id: str, max_retries: int = 3, initial_delay: float = 2.0) -> None:
        """
