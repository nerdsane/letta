...
class TrajectoryManager:
    ...
    async def get_trajectory_async(self, trajectory_id: str, actor: PydanticUser) -> Optional[Trajectory]:
        ...
        async with db_registry.async_session() as session:
            result = await session.execute(
                select(TrajectoryModel).where(TrajectoryModel.id == trajectory_id, TrajectoryModel.organization_id == actor.organization_id)
            )
    ...
    async def create_and_process_async(
        self, trajectory_create: TrajectoryCreate, actor: PydanticUser, auto_process: bool = True
    ) -> Trajectory:
        """
        ...
        if auto_process:
            task = asyncio.create_task(self._process_trajectory_background(trajectory.id, actor))
    ...
    async def _process_trajectory_background(
        self, trajectory_id: str, actor: PydanticUser, max_retries: int = 3, initial_delay: float = 2.0
    ) -> None:
        """
    ...
    async def get_processing_stats_async(self, actor: PydanticUser) -> Dict[str, int]:
        ...
        async with db_registry.async_session() as session:
            # Count by status
            result = await session.execute(
                select(TrajectoryModel.processing_status, func.count(TrajectoryModel.id).label("count"))
                .where(TrajectoryModel.organization_id == actor.organization_id)
