...
class OTSEntity(BaseModel):
    ...
    type: str = Field(..., description="Entity type (e.g., 'world', 'rule', 'character')")
    id: str = Field(..., description="Entity identifier")
    name: Optional[str] = Field(None, description="Human-readable name")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Type-specific attributes")
...
class OTSCreditAssignment(BaseModel):
    ...
    contribution_to_outcome: float = Field(..., ge=-1, le=1)
    pivotal: bool = False
