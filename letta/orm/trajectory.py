...
class Trajectory(SqlalchemyBase, OrganizationMixin):
    ...
    # Agent relationship
    agent_id: Mapped[str] = mapped_column(String, ForeignKey("agents.id"), nullable=False, doc="The agent that generated this trajectory")
    ...
    # Outcome scoring
    outcome_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True, doc="Quality score 0-1 (higher is better, LLM-generated)")
    score_reasoning: Mapped[Optional[str]] = mapped_column(Text, nullable=True, doc="Explanation of the outcome score (LLM-generated)")
    ...
    if settings.database_engine is DatabaseChoice.POSTGRES:
        ...
        embedding: Mapped[Optional[list]] = mapped_column(
            Vector(MAX_EMBEDDING_DIM), nullable=True, doc="Vector embedding for similarity search"
        )
    else:
    ...
    processing_status: Mapped[str] = mapped_column(
        String, nullable=False, default="pending", doc="Processing status: pending, processing, completed, failed"
    )
    processing_started_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, doc="When LLM processing started")
    processing_completed_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, doc="When LLM processing completed")
    processing_error: Mapped[Optional[str]] = mapped_column(Text, nullable=True, doc="Error message if processing failed")
