...
from ots.protocols import LLMClient
...
# Prompt for LLM entity extraction (used in full mode alongside decision extraction)
ENTITY_EXTRACTION_PROMPT = """Analyze this agent conversation and reasoning to identify entities mentioned or operated on.

Extract entities that are:
1. Services, systems, or applications mentioned
2. Files, documents, or resources referenced
3. People, users, or teams mentioned
4. Concepts, features, or components discussed
5. Domain-specific objects (worlds, stories, rules, etc.)
...
Output JSON array:
[{{
  "type": "service | file | user | concept | resource | custom_type",
  "id": "unique identifier if available, or descriptive slug",
  "name": "human-readable name",
  "metadata": {{
    "mentioned_in": "user_message | reasoning | tool_call | tool_result",
    "context": "brief description of how it was referenced"
  }}
}}]
...
class DecisionExtractor:
    """
    Extracts decisions and entities from agent turns.

    Combines programmatic extraction (tool calls) with optional LLM extraction (reasoning).

    Modes:
    - fast: Programmatic extraction only (free, immediate)
    - full: Programmatic + LLM extraction (extracts decisions AND entities)
    - deferred: Mark for later extraction

    Example:
        # Fast mode (programmatic only, decisions only)
        extractor = DecisionExtractor()
        decisions = extractor.extract_from_turn(turn, mode="fast")

        # Full mode with LLM (decisions AND entities)
        extractor = DecisionExtractor(llm_client=my_client)
        result = await extractor.extract_full(turn)
        print(result.decisions, result.entities)
    """
    ...
    def extract_from_trajectory(
        ...
        """
        Extract decisions from all turns in a trajectory.

        Args:
            trajectory: OTS trajectory to extract from
            mode: Extraction mode ("fast" or "deferred")

        Returns:
            Trajectory with decisions populated
        """
        for turn in trajectory.turns:
            if not turn.decisions:
                turn.decisions = self.extract_from_turn(turn, mode=mode)
    ...
    def extract_from_turn(
        ...
        """
        Extract decisions from a turn (sync version).
        ...
        """
        # Programmatic extraction from tool calls (always free)
    ...
    async def extract_from_turn_async(
        ...
        """
        Extract decisions from a turn (async version with LLM support).
        ...
        if self.llm_client and mode == "full":
            ...
            if reasoning:
                return await self._extract_with_llm(turn, tool_decisions, reasoning)
    ...
    async def extract_full(
        ...
        This is the recommended method for full context learning, as it extracts
        entities that can populate context_snapshot.entities.

        Args:
            turn: OTS turn to extract from

        Returns:
            ExtractionResult with decisions and entities

        Raises:
            ValueError: If no LLM client is configured
        """
        if not self.llm_client:
            raise ValueError("LLM client required for full extraction")

        # Step 1: Programmatic extraction of decisions from tool calls
        tool_decisions = self._extract_from_tool_calls(turn)

        # Step 2: Gather context for LLM extraction
        user_message = self._extract_user_message(turn)
        reasoning = self._extract_reasoning_text(turn)
        tool_calls_text = self._format_tool_calls(turn)

        # Step 3: Use LLM to extract both decisions and entities
        ...
        try:
            ...
            # Extract decisions
            llm_decisions = result.get("decisions", [])
            merged_decisions = self._merge_decisions(
                tool_decisions, llm_decisions, turn.turn_id
            )
    ...
    def _extract_user_message(self, turn: OTSTurn) -> Optional[str]:
        ...
        for msg in turn.messages:
            if msg.role.value == "user" and msg.content.text:
                return msg.content.text[:2000]  # Truncate for prompt
        return None

    def _format_tool_calls(self, turn: OTSTurn) -> str:
        """Format tool calls and results for the prompt."""
        parts = []

        for msg in turn.messages:
            if msg.content.type == ContentType.TOOL_CALL:
                tool_calls = msg.content.data.get("tool_calls", []) if msg.content.data else []
                for tc in tool_calls:
                    func = tc.get("function", {})
                    parts.append(f"- {func.get('name', 'unknown')}: {func.get('arguments', {})}")

            elif msg.content.type == ContentType.TOOL_RESPONSE:
    ...
    def _find_tool_consequence(
        ...
        for msg in turn.messages:
            if msg.content.type != ContentType.TOOL_RESPONSE:
                continue
